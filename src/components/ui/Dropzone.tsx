...
import { deleteImage } from '@/app/api/actions/actions';
...
export function Dropzone() {
  ...
  const uploadFile = async (file: File) => {
    try {
      // we upload eveything right here
  ...
  const removeFile = async (fielId: string, fielName: string) => {
    if(fielId){
      ...
      if(result.success){
        ...
        toast.success(`File ${fielName} deleted succesfully`)
      } else {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    if(acceptedFiles.length){
      setFiles((prevFiels) => [...prevFiels, ...acceptedFiles.map((file) => ({file, uploading: false})),
      ]);
  ...
  const rejectectedFiles = useCallback((fileRejection: FileRejection[]) => {
    if(fileRejection.length){
      const toomanyFiles = fileRejection.find((rejection) => rejection.errors[0].code 
      === "too-many-files");

      const fileSizetoBig = fileRejection.find((rejection) => rejection.errors[0].code 
      === "file-too-large");

      if(toomanyFiles){
        toast.error("Too may files selected, max is 5")
      }
      if(fileSizetoBig){
  ...
  const {getRootProps, getInputProps, isDragActive} = useDropzone({
    onDrop,
    onDropRejected: rejectectedFiles,
     maxFiles: 5,
        ...
        <div key={file.name} className='relative w-full group'>
          <div className='relative'>
            ...
            className={cn(
              ...
              'rounded-lg object-cover size-40'
            )}
            />
          ...
          <p className='mt-2 text-sm text-gray-400 truncate'>{file.name}</p>
        </div>
      ))}
    </div>
    </>
